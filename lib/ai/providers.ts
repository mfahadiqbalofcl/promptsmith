...
// OpenAI-compatible providers: Groq and OpenRouter. Whichever keys are present
// become the chain, tried in order until one succeeds — so when one hits a rate
// limit or errors, the next takes over. That keeps AI features up 24/7 on free tiers.
...
// Both are overridable via their env var if a provider rotates its catalog.
...
const DEFAULT_ORDER: ProviderId[] = ["groq", "openrouter"];
